...
function TRAIL:Data()
	return warptorio.platform.rails[self.key]
end

function TRAIL:MakeRails()
	local f = warptorio.GetMainSurface()
	local tps = self:Data()
	if (not isvalid(self.rails[1]) or not isvalid(self.rails[2])) then
		vector.clean(f,
			vector.square(tps.railpos, vector(1, 1)))
	end
	if (not isvalid(self.rails[1])) then
		self.rails[1] = entity.protect(
			entity.spawn(f, "straight-rail", tps.railpos, defines.direction.south), false, false)
	end
	if (not isvalid(self.rails[2])) then
		self.rails[2] = entity.protect(
			entity.spawn(f, "straight-rail", tps.railpos, defines.direction.east), false, false)
	end
end
...
function TRAIL:MakeChests()
	...
	for i, px in pairs(vector.compasscorn) do
		...
		if (isvalid(v) and v.name ~= ccls) then
			self.chestcontents[i] = v.get_inventory(defines.inventory.chest).get_contents()
		...
		if (not isvalid(v)) then
			...
			if (inv) then
				...
				for x, y in pairs(inv) do cv.insert { name = x, count = y } end
				self.chestcontents[i] = nil
			end
		end
...
function TRAIL:MakeLoaders()
	...
	for i, b in pairs(tps.logs) do
		if (b) then
			local cd = vector.compass[string.compass[i]] * 2
			for x = 1, 2, 1 do
				local v = self.loaders[i][x]
				if (isvalid(v) and v.name ~= bcls) then
					entity.destroy(v)
				end
				if (not isvalid(v)) then
					local vang = (((i - 1) * 2) + 4) % 8
					local vpos = tps.chestpos + cd +
						vector.compassall[string.compassall[((vang + 2) % 8) + 1]] * (x == 1 and 0.5 or -0.5)
					local varea = vector.square(vpos, vector((i == 1 or i == 3) and 0.5 or 1, (i == 1 or i == 3) and 1 or
						0.5))
					vector.clean(f, varea)
					v = entity.protect(entity.create(f, bcls, vpos, (vang * 2 + 8) % 16), false, false)
					v.loader_type = self.dir
					self.loaders[i][x] = v
				end
...
function TRAIL:SplitItem(u, n)
	...
	for k, v in pairs(self.chests[1]) do
		local iv = v.get_inventory(defines.inventory.chest)
		if (iv.can_insert(u)) then
			cinv[k] = iv
		end
	end
	local tcn = table_size(cinv)
	for k, v in pairs(cinv) do
		if (c > 0) then
			local w = v.insert { name = u, count = math.ceil(c / tcn) }
			cx = cx + w
			c = c - w
			tcn = tcn - 1
		end
...
function TRAIL:UnloadLogistics(e)
	for _, r in pairs(e) do
		...
		for k, v in pairs(inv.get_contents()) do
			local ct = self:SplitItem(v.name, v.count)
			if (ct > 0) then inv.remove({ name = v.name, count = ct }) end
		end
	end
end
...
function TRAIL:LoadLogistics(e)
	local inv = {}
	for k, v in pairs(self.chests[1]) do inv[k] = v.get_inventory(defines.inventory.chest) end
	local ct = {}
	for k, v in pairs(inv) do
		for a, b in pairs(v.get_contents()) do ct[a] = (ct[a] or 0) + b end
		v.clear()
	end
	for _, r in pairs(e) do
	...
	local ci
	for a, b in pairs(ct) do
		local g = b
		ci = #inv
		for k, v in pairs(inv) do
			if (ci > 0) then
				local gci = math.ceil(g / ci)
				if (gci > 0) then
					local w = v.insert { name = a, count = math.ceil(g / ci) }
					ci = ci - 1
					g = g - w
				end
			end
...
function TRAIL:BalanceChests()
	local inv = {}
	for k, v in pairs(self.chests[1]) do if (isvalid(v)) then inv[k] = v.get_inventory(defines.inventory.chest) end end
	if (table_size(inv) > 0) then
		local ct = {}
		for k, v in pairs(inv) do
			for a, b in pairs(v.get_contents()) do ct[b] = (ct[b] or 0) + b.count end
			v.clear()
		end
		local ci
		for a, b in pairs(ct) do
			...
			for k, v in pairs(inv) do
				local gci = math.ceil(g / ci)
				if (gci > 0) then
					local w = v.insert { name = a.name, count = math.ceil(g / ci) }
					ci = ci - 1
					g = g - w
				end
			end
...
function TRAIL:TickLogistics()
	local f = storage.floor.main.host
	if (not f.valid) then return end
	local c = self:Data().railpos
	local e = f.find_entities_filtered { name = "cargo-wagon", area = { { c.x - 1, c.y - 1 }, { c.x + 1, c.y + 1 } } }
	if (table_size(e) > 0) then
		if (self.dir == "output") then
			self:UnloadLogistics(e)
			self:BalanceChests()
		else
			self:LoadLogistics(e)
		end
	else
		self:BalanceChests()
	end
