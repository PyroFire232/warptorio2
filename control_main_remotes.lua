...
warptorio.remote.GetSurface=function(key) return (storage.floor[key] and storage.floor[key].host or nil) end
warptorio.remote.GetFloor=function(key) return storage.floor[key] end
warptorio.remote.GetFloors=function() return storage.floor end
...
warptorio.remote.RecallTeleporterGate=function() local t=storage.Teleporters.offworld if(t and isvalid(t.b))then t:DestroyLogsB() t:DestroyB() end end -- call(). Used to destroy the teleporter gate.
warptorio.remote.RecallHarvester=function(side,bply) if(storage.Harvesters[side])then storage.Harvesters[side]:Recall(bply) end end -- call(string_side,bool_recallplayers). side must be "east" or "west". future values may include "nw","ne","sw","se" if i ever add those.
...
function warptorio.cmdinsertcloneblacklist(mn,e) if(not storage.warp_blacklist[mn])then storage.warp_blacklist[mn]={} end table.insertExclusive(storage.warp_blacklist[mn],e) end
function warptorio.cmdremovecloneblacklist(mn,e) if(not storage.warp_blacklist[mn])then storage.warp_blacklist[mn]={} end table.RemoveByValue(storage.warp_blacklist[mn],e) end
function warptorio.cmdiscloneblacklisted(mn,e) if(not storage.warp_blacklist[mn])then return false end return table.HasValue(storage.warp_blacklist[mn],e) end
]]


function warptorio.ValidateRemoteTable(x) local mt=storage.modtbl if(not mt[x])then mt[x]={} end
	for k,v in pairs(mt[x])do if(not script.active_mods[k])then mt[x][k]=nil end end
end
function warptorio.InsertModTable(x,y,z) if(y=="warptorio2")then return end local mt=storage.modtbl if(not mt[x])then return false end mt[x][y]=mt[x][y] or {}
	return table.insertExclusive(mt[x][y],z)
end
function warptorio.RemoveModTable(x,y,z) if(y=="warptorio2")then return end local mt=storage.modtbl if(not mt[x])then return false end mt[x][y]=mt[x][y] or {}
...
function warptorio.GetModTable(x)
	...
	local mt=storage.modtbl if(not mt[x])then return true end
...
events.on_config(function()
	storage.modtbl=storage.modtbl or {}
...
warptorio.remote.reveal=function(n) n=n or 10 local f=storage.floor.main.host game.forces.player.chart(f,{lefttop={x=-64-128*n,y=-64-128*n},rightbottom={x=64+128*n,y=64+128*n}}) end -- call (reveal_scale), returns: nil. Cheat command to reveal the map
...
warptorio.remote.ResetPlatform=function() warptorio.BuildB1() warptorio.BuildB2() for k,v in pairs(storage.Teleporters)do v:Warpin() end end -- Rebuild the platform??


---------------------------------------------------------------------
--[[ Warping related stuff ]]--

warptorio.remote.GetWarpzone=function() return storage.warpzone end -- Returns the current warpzone
...
function warptorio.remote.GetWarpTime() return (storage.warp_charging>0 and storage.warp_time_left/60 or storage.warp_charge_time) end
function warptorio.remote.SetWarpTime(n) storage.warp_charge_time=n end
