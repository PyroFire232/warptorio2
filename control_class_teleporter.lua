...
function warptorio.GetPlatformTechLevel(nm) local tc=warptorio.platform.techs[nm] if(not tc)then return false end
	if(tc.level_range or tc.levels)then return research.level(tc.tech) else return research.has(tc.tech) end
end
function warptorio.GetPlatformTechAmount(nm) local tc=warptorio.platform.techs[nm] if(not tc)then return false end
...
function warptorio.GetPlatformResearches() -- cache stuff
	...
	for vi,v in pairs(warptorio.platform.techs)do
		v.key=v.key or vi
		if(v.levels)then for k in pairs(v.levels)do warptorio.PlatformResearches[v.tech.."-"..k]=v end
		elseif(v.level_range)then for i=v.level_range[1],v.level_range[2] do warptorio.PlatformResearches[v.tech.."-"..i]=v end
		else warptorio.PlatformResearches[v.tech]=v
		end
	end
...
function warptorio.ResearchFinished(ev)
	...
	if(u)then
		...
		if(u.lv_effect)then local lvt=u.lv_effect[lv] if(lvt)then warptorio.DoResearchEffects(lvt,lv) end end
...
function TELL:CheckTeleporterPairs(bSound) -- Call updates and stuff. Automatically deals with logistics, upgrades and cleaning as-needed with good accuracy
	local tps=self:Data()
	if(tps.pair)then for i,t in pairs(tps.pair)do local pi=self.points[i] self:MakePointTeleporter(tps,i,t,(t.gate and isvalid(pi.ent)) and pi.ent.position or nil) end end
	local ca=cache.get_entity(self.points[1].ent)
	local cb=cache.get_entity(self.points[2].ent)
	if(ca and cb)then ca.teleport_dest=cb.host cb.teleport_dest=ca.host else if(ca)then ca.teleport_dest=nil end if(cb)then cb.teleport_dest=nil end end
...
function TELL:GetLoaderDirection() local tps=self:Data()
	...
	return (tps.staticdir and tps.staticdir or (tps.top and warptorio.setting("loader_top") or warptorio.setting("loader_bottom"))) or "up"
end
...
function TELL:RemakeChestPair(o,k) local e=self.chests[o][k] local ex=warptorio.GetChest(self.dir[o][k])
	if(e and e.name~=ex)then 
		local v=entity.protect(entity.create(e.surface,ex,e.position),false,false) entity.copy.chest(e,v)
		entity.destroy(e) self.chests[o][k]=v if(self.dir[o][k]=="input")then entity.ChestRequestMode(v) end
		cache.get_raise_type("types",v.type,v,"Teleporters",self.key,"chests",o,k)
	end
end
function TELL:SwapLoaderChests(id) self:RemakeChestPair(1,id) self:RemakeChestPair(2,id) end
...
function TELL:MakePointLoader(tps,i,id,ido,pos,f,belt,lddir,chesty,belty,vexdir)
	...
	if(not isvalid(v))then
		...
		if(inv)then local cv=v.get_inventory(defines.inventory.chest) for x,y in pairs(inv)do cv.insert{name=x,count=y} end self.chestcontents[i][id]=nil end
...
function TELL:MakePointLoaders(tps,i,id,pos,f,belt,lddir,chesty,belty)
	if(not tps.oneside or tps.oneside=="right")then self:MakePointLoader(tps,i,id,id,pos,f,belt,lddir,chesty,belty,1) end
	if(not tps.oneside or tps.oneside=="left")then self:MakePointLoader(tps,i,id+3,id,pos,f,belt,lddir,chesty,belty,-1) end
end
function TELL:MakePointPipes(tps,i,id,pos,f,dist,vexdir,ido) -- TODO: Initial pipe dir & remember direction
	...
	local pipedir=(vexdir==1 and 2 or 6)
	if(isvalid(v) and (v.surface~=f or v.position.x~=vpos.x or v.position.y~=vpos.y))then pipedir=v.direction entity.destroy(v) end
	if(not isvalid(v))then
		local varea=vector.square(vpos,vector(0.5,0.5))
		vector.clean(f,varea)
		v=entity.protect(entity.create(f,pipe,vpos,pipedir),false,false)
		vector.cleanplayers(f,varea)
		self.pipes[i][id]=v
	end
...
function TELL:CheckPointLogistics(i,vxpos)
	...
	if(can and ldl>0)then for id=1,ldl,1 do
		self:MakePointLoaders(tps,i,id,pos,f,belt,lddir,chesty,belty)
	end end
	if(can and tps.dopipes and lvLogs>0)then for id=1,math.min(lvLogs,3),1 do -- pipes first, it removes the old ones
		if(not tps.oneside or tps.oneside=="right")then self:MakePointPipes(tps,i,id,pos,f,ldl+offld+1,1,id) end
		if(not tps.oneside or tps.oneside=="left")then self:MakePointPipes(tps,i,id+3,pos,f,ldl+offld+1,-1,id) end
	end end
end
