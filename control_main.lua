...
function warptorio.GetChest(dir)
	local lv = research.level("warptorio-logistics")
	local lvb = warptorio.ChestBeltPairs[lv]
	return (isstring(lvb[2]) and lvb[2] or lvb[2](dir))
end

function warptorio.GetBelt(dir)
	local lv = research.level("warptorio-logistics")
	local lvb = warptorio.ChestBeltPairs[lv]
	return (isstring(lvb[1]) and lvb[1] or lvb[1](dir))
end
...
function warptorio.GetCurrentSurface() return storage.floor.main.host end

function warptorio.GetMainSurface() return storage.floor.main.host end

function warptorio.GetHomeSurface() return storage.floor.home and storage.floor.home.host or nil end
...
function warptorio.GetNamedSurfaces(tbl)
	local t = {}
	for k, nm in pairs(tbl) do t[nm] = storage.floor[nm].host end
	return t
end

function warptorio.GetAllSurfaces()
	local t = {}
	for nm, v in pairs(storage.floor) do t[v.hostindex] = v.host end
	return t
end

function warptorio.GetPlatformSurfaces()
	local t = {}
	for nm, v in pairs(storage.floor) do if (platform.floors[nm].empty == true) then t[v.hostindex] = v.host end end
	return t
end
...
function warptorio.GetTeleporterHazard(bMain, bFull) -- for hazard tiles
	...
	if (bMain) then
		if (tlHas and dlHas) then
			x = x + 2
		elseif (tlHas or dlHas) then
			x = x + 1
			if (bFull and ((not tlHas and tlCan) or (not dlHas and dlCan))) then x = x + 1 end
		elseif (bFull and (tlCan or dlCan)) then
			x = x + 1
		end
	else
...
function warptorio.GetPlatformFloor(vt)
	if (isstring(vt)) then vt = warptorio.platform.floors[vt] end
	local floor
	if (vt.key == "main") then -- Special; nauvis / primary planet
		floor = storage.floor.main
		if (not floor) then
			floor = { key = vt.key, host = game.surfaces.nauvis, hostindex = 1 }
			storage.floor.main = floor
			if (vt.init) then lib.call(vt.init, floor) end
		end
	elseif (vt.key == "home") then -- Special; homeworld
		floor = storage.floor.home
		if (not floor) then
			floor = { key = vt.key, host = game.surfaces.nauvis, hostindex = 1 }
			storage.floor.home = floor
		end
	else
		floor = storage.floor[vt.key]
		if (not floor) then floor = warptorio.MakePlatformFloor(vt) end
		floor.key = vt.key
	end
	return floor
end
...
function warptorio.ConstructPlatformVoid(surf)
	local vt = warptorio.platform.floors["main"]
	if (vt.tile) then lib.call(vt.tile, surf, true) end
end
...
function warptorio.CheckFloorRadar(floor)
	if (research.has("warptorio-charting") and not isvalid(floor.radar)) then
...
function warptorio.CheckPlatformSpecials(self)
	...
	local inv = {}
	if (sp.upgrade) then
		protoname = protoname .. "-" .. research.level(sp.tech)
		if (isvalid(self.SpecialEnt) and self.SpecialEnt.name == protoname) then
			return
		elseif (isvalid(self.SpecialEnt)) then
			inv =
				warptorio.DestroyPlatformSpecial(self)
		end
	end
	...
	if (isvalid(self.radar)) then
		rdr = true
		entity.destroy(self.radar)
	end
	for k, v in pairs(eft) do
		if (isvalid(v)) then
			if (v.type == "character") then
				table.insert(efply, v)
			elseif (v ~= self.radar) then
				entity.destroy(v)
			end
		end
	end
	...
	self.SpecialEnt = e
	if (inv) then for k, v in pairs(inv) do e.get_module_inventory().insert { name = v.name, count = v.count } end end -- beacon modules. Close enough.
	warptorio.CheckFloorRadar(self)
	vector.cleanplayers(f, area)
	players.playsound("warp_in", f)
end

function warptorio.DestroyPlatformSpecial(self)
	local inv
	if (isvalid(self.SpecialEnt)) then
		local x = self.SpecialEnt.get_module_inventory()
		if (x) then inv = x.get_contents() end
		entity.destroy(self.SpecialEnt)
	end
	self.SpecialEnt = nil
	return inv
end
...
function warptorio.ApplyMapSettings()
	...
	gmp.enemy_evolution.pollution_factor = 0.0000001
	...
	gmp.enemy_expansion.settler_group_min_size = 1
	gmp.enemy_expansion.settler_group_max_size = 1

	--gmp.enemy_expansion.max_expansion_cooldown = (gmp.enemy_expansion.min_expansion_cooldown*1.25)
end

events.on_config(function(ev)
	if (warptorio.Loaded) then return end
	lib.planets.lua()
	...
	if (type(storage.votewarp) ~= "table") then storage.votewarp = {} end
...
function warptorio.CheckVotewarps()
	for k, v in pairs(storage.votewarp) do if (isvalid(v) or not v.connected) then storage.votewarp[k] = nil end end
...
function warptorio.TeleportLogic(ply, e, tent)
	if ply.controller_type == defines.controllers.remote then return end
	local w = ply.walking_state
	local ox = tent.position
	local x = e.position
	local mp = 2
	if (not ply.character) then mp = 3 end
	--game.print(serpent.line(ply.vehicle.name))
	if (ply.driving) then
		local veh = ply.vehicle
		if (veh.type == "spider-vehicle") then
			--[[	local cp = ply.position
			local xd, yd = (x.x - cp.x), (x.y - cp.y)
			...
			local cp = ply.position
			local xd, yd = (x.x - cp.x), (x.y - cp.y)
			entity.safeteleport(veh, tent.surface, vector(ox.x + xd, ox.y + yd))
		end
	elseif (not w.walking) then
		local cp = ply.position
		local xd, yd = (x.x - cp.x), (x.y - cp.y)
		entity.safeteleport(ply, tent.surface, vector(ox.x + xd * mp, ox.y + yd * mp))
	else
		local cp = ply.position
		local xd, yd = (x.x - cp.x), (x.y - cp.y)
		entity.safeteleport(ply, tent.surface, vector(ox.x + xd * mp, ox.y + yd * mp))
	...
	players.playsound("teleport", e.surface, e.position)
	players.playsound("teleport", tent.surface, tent.position)
end
...
cache.player({
	...
	on_respawn = function(ply)
		...
		local gp = ply
		if (gp.surface ~= cf) then
			local pos = cf.find_non_colliding_position("character", { 0, -5 }, 0, 1, 1)
			gp.teleport(pos, cf)
		end
	end,
...
cache.ent("warptorio-heatpipe",
	{ create = function(e) cache.insert("heat", e) end, destroy = function(e) cache.remove("heat", e) end })
cache.ent("warptorio-reactor",
	{ create = function(e) cache.insert("heat", e) end, destroy = function(e) cache.remove("heat", e) end })
cache.ent("warptorio-accumulator",
	{ create = function(e) cache.insert("power", e) end, destroy = function(e) cache.remove("power", e) end })

events.on_tick(1, 0, "heattick", function(tick) entity.AutoBalanceHeat(cache.get("heat")) end)
events.on_tick(1, 0, "powertick", function(tick)
	local t = cache.get("power")
	local g, c = 0, 0
	...
	if (abc > 0) then
		...
		if (g >= gcost) then
			...
			storage.abilities.stabilizing = false
			storage.abilities.scanning = false
			storage.abilities.accelerating = false
		end
	end
	storage.energycount = g
	storage.energymax = c
...
cache.ent("warptorio-combinator", {
	...
	update = function(e, ev)
		...
		for k, v in pairs(ev.signals) do cbh.set_signal(k, v) end
	end,
})
function warptorio.RefreshWarpCombinators()
	local sigs = warptorio.GetCombinatorSignals()
	cache.entcall("combinators", "update", { signals = sigs })
end

function warptorio.GetCombinatorSignals()
	local tbl = {}
...
warptorio.Signals[2] = {
	...
	get = function()
		return storage.warp_charging or
			0
	end
}
warptorio.Signals[3] = {
...
warptorio.Signals[4] = {
	...
	get = function()
		...
		return ((hv and hv.deployed) and 1 or 0)
	end
}
warptorio.Signals[5] = {
...
function warptorio.RaiseHUD(v)
	local m = cache.get_menu("hud", v)
	if (not m) then cache.raise_menu("hud", v) else cache.call_menu("raise", m) end
end

function warptorio.ResetHUD(p)
	if (not p) then
		for k, v in pairs(game.players) do warptorio.RaiseHUD(v) end
	else
		warptorio.RaiseHUD(p)
	end
end

function warptorio.PlayerCanStartWarp(ply) return true end

function warptorio.ToolRecallHarvester(k, ply)
	if (not research.has("warptorio-harvester-" .. k .. "-1")) then return end
	local cn = ("warptorio-harvestpad-" .. k .. "-" .. research.level("warptorio-harvester-" .. k))
	if (not ply or (ply and not ply.get_main_inventory().get_contents()[cn])) then
		ply.get_main_inventory().insert { name = cn, count = 1 }
		players.playsound("warp_in", ply.surface, ply.position)
	end
	local hv = storage.Harvesters[k]
	if (hv and hv.deployed and isvalid(hv.b)) then
		players.playsound("warp_in", hv.b.surface, hv.b.position)
		hv:Recall()
		hv:DestroyB()
	end
end

function warptorio.ToolRecallGate(ply)
	if (not research.has("warptorio-teleporter-portal")) then return end
	local t = storage.Teleporters.offworld
	if (t) then
		if (t.b and t.b.valid) then
			players.playsound("warp_in", t.b.surface, t.b.position)
			t:DestroyLogsB()
			t:DestroyB()
		end
		local inv = ply.get_main_inventory()
		if (not inv.get_contents()["warptorio-teleporter-gate-0"]) then
			inv.insert { name = "warptorio-teleporter-gate-0", count = 1 }
			players.playsound("warp_in", ply.surface, ply.position)
		end
	end
end
...
cache.vgui("warptorio_toolbutton",
	{
		click = function(elm, ev)
			local menu = cache.get_menu("hud", elm.player_index)
			local b = menu.toolbar
			b.visible = not b.visible
		end
	})
cache.vgui("warptorio_tool_hv_west",
	{ click = function(elm, ev) warptorio.ToolRecallHarvester("west", game.players[elm.player_index]) end })
cache.vgui("warptorio_tool_hv_east",
	{ click = function(elm, ev) warptorio.ToolRecallHarvester("east", game.players[elm.player_index]) end })
cache.vgui("warptorio_tool_planet_gate",
	{ click = function(elm, ev) warptorio.ToolRecallGate(game.players[elm.player_index]) end })
...
cache.vgui("warptorio_homeworld", {
	click = function(elm, ev)
		...
		if (menu.hometmr < game.tick) then
			...
			storage.homeworld = storage.warpzone
			local f = storage.floor.main.host
			storage.floor.home.host = f
			storage.floor.home.hostindex = f.index
			players.playsound("warp_in", storage.floor.home.host)
...
local HUD = {}
warptorio.HUD = HUD
function HUD.clocktick(menu)
	local ply = menu.host
	if (storage.warp_charging >= 1) then
		menu.charge_time.caption = { "warptorio.warp-in", util.formattime(val or (storage.warp_time_left or 0)) }
	elseif (menu.charge_time) then
		menu.charge_time.caption = { "warptorio.charge_time", util.formattime((storage.warp_charge_time or 0) * 60) }
	end
	...
	if (warptorio.IsAutowarpEnabled()) then
		menu.autowarp.caption = { "warptorio.autowarp-in", util.formattime(storage.warp_auto_end) }
	else
		menu.autowarp.caption = ""
	end

	menu.hometmr = menu.hometmr or 0
	if (menu.homeworld) then
		if (menu.hometmr > game.tick) then
			menu.homeworld.caption = { "warptorio.confirm_homeworld", util.formattime(menu.hometmr - game.tick) }
		else
			menu.homeworld.caption = { "warptorio.button-homeworld" }
		end
	end
	...
	if (menu.energybar) then
		...
		if (r) then
			if (storage.abilities.stabilizing) then
				abc = abc + 1
				r.caption = { "warptorio.stabilize-on", "-" .. math.roundx(egdrain, 2) .. "%/sec" }
			else
				r.caption = { "warptorio.stabilize" }
			end
		end
		...
		if (r) then
			if (storage.abilities.accelerating) then
				abc = abc + 1
				r.caption = { "warptorio.accel-on", "-" .. math.roundx(egdrain, 2) .. "%/sec" }
			else
				r.caption = { "warptorio.accel" }
			end
		end
		local r = menu.radar
		if (r) then
			if (storage.abilities.scanning) then
				abc = abc + 1
				r.caption = { "warptorio.radar-on", "-" .. math.roundx(egdrain, 2) .. "%/sec" }
			else
				r.caption = { "warptorio.radar" }
			end
		end
		...
		menu.energybar_energybal.caption = " (" ..
			(energydif >= 1 and "+" or (energydif > 0 and "+-" or "")) .. string.energy_to_string(energydif) .. "/sec) "
		...
		if (abc > 0) then
			menu.energybar_energypctx.caption = "-" .. math.roundx(egdrain * abc, 2) .. "%/sec"
		else
			menu.energybar_energypctx.caption = ""
		end
	end
end

function HUD.raise(menu, ev)
	local ply = menu.host
	...
	menu.warpbtn = vgui.create(menu.row1,
		{ name = "warptorio_warpbutton", type = "button", caption = { "warptorio.button-warp", "-" } })
	if (research.has("warptorio-toolbar")) then
		menu.toolbtn = vgui.create(menu.row1,
			{ name = "warptorio_toolbutton", type = "button", caption = { "warptorio.toolbutton", "-" } })
	end
	if (research.level("warptorio-reactor") >= 8) then
		...
		HUD.warptarget(menu,
			{ tgt = (sx == nil and "(Random)" or (sx == "home" and "(Homeworld)" or (sx == "(nauvis)" and "nauvis" or sx))) })
	end
	...
	menu.warpzone = vgui.create(menu.row1,
		{ name = "warptorio_warpzone", type = "label", caption = { "warptorio.warpzone", storage.warpzone or 0 } })
	menu.autowarp = vgui.create(menu.row1, { name = "warptorio_autowarp", type = "label" })

	if (research.has("warptorio-homeworld")) then
		menu.homeworld = vgui.create(menu.row1,
			{ name = "warptorio_homeworld", type = "button", caption = { "warptorio.button-homeworld" } })
	end


	local hasabil = false
	if (research.has("warptorio-stabilizer")) then
		hasabil = true
		menu.stabilizer = vgui.create(menu.row2,
			{ name = "warptorio_stabilizer", type = "button", caption = { "warptorio.stabilize", "-" } })
	end
	if (research.has("warptorio-accelerator")) then
		hasabil = true
		menu.accelerator = vgui.create(menu.row2,
			{ name = "warptorio_accelerator", type = "button", caption = { "warptorio.accel", "-" } })
	end
	if (research.has("warptorio-charting")) then
		hasabil = true
		menu.radar = vgui.create(menu.row2,
			{ name = "warptorio_radar", type = "button", caption = { "warptorio.radar", "-" } })
	end
	if (hasabil) then
		menu.last_energy = menu.last_energy or 100
		local energydif = 1000 - menu.last_energy
		menu.energybar_label = vgui.create(menu.row4,
			{ name = "warptorio_energybar_label", type = "label", caption = { "warptorio.energybar", "-" } })
		menu.energybar_energy = vgui.create(menu.row4, {
			name = "warptorio_energybar_energy",
			type = "label",
			caption =
			" 100kw "
		})
		...
		menu.energybar_energydiv = vgui.create(menu.row4,
			{ name = "warptorio_energybar_energydiv", type = "label", caption = " / " })

		menu.energybar_energymax = vgui.create(menu.row4,
			{ name = "warptorio_energybar_energymax", type = "label", caption = " 0kw " })
		menu.energybar_energymax.style.font_color = { r = 0.25, g = 1, b = 1 }


		menu.energybar_energybal = vgui.create(menu.row4,
			{ name = "warptorio_energybar_energybal", type = "label", caption = " (+100.32MW/sec) " })
		menu.energybar_energybal.style.font_color = (energydif > 0 and { r = 0, g = 1, b = 0 } or (energydif == 0 and { r = 1, g = 1, b = 0 } or { r = 1, g = 0, b = 0 }))


		menu.energybar_energydivb = vgui.create(menu.row4,
			{ name = "warptorio_energybar_energydivb", type = "label", caption = " | " })
		...
		menu.energybar_energypcta = vgui.create(menu.row4,
			{ name = "warptorio_energybar_energypcta", type = "label", caption = " | " })
		menu.energybar_energypct = vgui.create(menu.row4,
			{ name = "warptorio_energybar_energypct", type = "label", caption = "25%" })
		menu.energybar_energypct.style.font_color = { r = 1, g = 1, b = 1 }
		menu.energybar_energypctx = vgui.create(menu.row4,
			{ name = "warptorio_energybar_energypctx", type = "label", caption = "-3%/sec" })
		menu.energybar_energypctx.style.font_color = { r = 1, g = 0, b = 0 }
	end

	if (research.has("warptorio-toolbar")) then
		menu.toolbar = vgui.create(menu.row3,
			{ name = "warptorio_toolframe", type = "flow", direction = "horizontal", visible = false })
		menu.toolbar.clear()
		menu.tool_harvester_west = vgui.create(menu.toolbar,
			{ name = "warptorio_tool_hv_west", type = "sprite-button", sprite = "entity/warptorio-harvestportal-1", tooltip = { "warptorio.tool_hv_west", "-" } })
		menu.tool_planet_gate = vgui.create(menu.toolbar,
			{ name = "warptorio_tool_planet_gate", type = "sprite-button", sprite = "item/warptorio-teleporter-gate-0", tooltip = { "warptorio.tool_planet_gate", "-" } })
		menu.tool_harvester_east = vgui.create(menu.toolbar,
			{ name = "warptorio_tool_hv_east", type = "sprite-button", sprite = "entity/warptorio-harvestportal-1", tooltip = { "warptorio.tool_hv_east", "-" } })
	end
...
function HUD.rebuild_warptargets(menu, ev)
	if (menu.warptgt) then
		local tgl = { "(Random)" }
		if (research.has("warptorio-homeworld")) then
			table.insert(tgl, "(Homeworld)")
			table.insert(tgl, "(Nauvis)")
		end
		if (research.has("warptorio-charting")) then
			for k, v in pairs(lib.planets.GetTemplates()) do
				table.insert(tgl, v.key)
			end
		end
...
function HUD.warptarget(menu, ev)
	local ply = menu.host
	if (not menu.warptgt or ply.index == ev.ply) then return end
	local elm, items = menu.warptgt
	if (elm) then items = elm.items end
...
function HUD.warpbtn(menu)
	...
	local ply = menu.host
	if (storage.warp_charging >= 1) then
		r.caption = { "warptorio.warping", "-" }
		r.enabled = false
	else
		local cx = table.Count(storage.votewarp)
		local c = table.Count(game.connected_players) -- table.Count(game.non_afk_players)
		if (c > 1) then
			...
			if (cx > 0) then
				r.caption = { "warptorio.button-votewarp-count", cx, vcn }
			else
				r.caption = { "warptorio.button-votewarp", "-" }
			end
		else
			r.enabled = true
			r.caption = { "warptorio.button-warp", "-" }
			menu.warpzone.caption = { "warptorio.warpzone_label", storage.warpzone or 0 }
		end
	end
end
...
cache.vgui("warptorio_warptarget", {
	selection_changed = function(elm, ev)
		local ply = game.players[elm.player_index]
		local s = elm.items[elm.selected_index]
		...
		if (storage.warp_charging < 1) then
			...
			if (c > 1 and warptorio.setting("votewarp_multi") > 0) then --votewarp
				...
				if (vcn <= 1 or cx >= vcn) then
					warptorio.StartWarp()
					game.print({ "warptorio.player_started_warpout", ply.name })
				else
			...
			elseif (warptorio.PlayerCanStartWarp(ply)) then
				...
				ply.print({ "warptorio.same-planet-error" })
			end
		end
	end,
})
...
events.on_event(defines.events.on_entity_cloned, function(ev)
	if (warptorio.IsCloning) then
		table.insert(warptorio.Cloned_Entities, {
			source = ev.source,
	...
	if (ev.source.type == "spider-vehicle") then
		for k, v in pairs(game.players) do
			if (v.driving and v.vehicle == ev.source) then
				...
				ev.destination.set_driver(v)
			end
			--[[
			local inv = v.get_main_inventory()
			if (inv) then
				for i = 1, #inv, 1 do
					local e = inv[i]
					if (e and e.valid_for_read and e.connected_entity == ev.source) then
						e.connected_entity = ev.destination
					end
				end
				local e = v.cursor_stack
				if (e and e.valid_for_read and e.connected_entity == ev.source) then e.connected_entity = ev.destination end
			end]]
		end
	end
end)
...
function warptorio.WarpPreBuildPlanet(key)
	...
	storage.warp_charge_time = math.min(
		10 + (cot / sgFactor) + (storage.warpzone * sgMul) +
		(sgZoneGain * (math.min(storage.warpzone, sgZone) / sgZone) * 60),
		60 * sgMax)
...
function warptorio.WarpBuildPlanet(key)
	local nplanet
	if (key) then
		nplanet = remote.call("planetorio", "FromTemplate", "warpzone_" .. storage.warpzone, key)
	else
		...
		if (lvl >= 8) then
			...
			elseif (wx and math.random(1, 100) <= warptorio.setting("warpchance")) then
				nplanet = remote.call("planetorio", "FromTemplate", "warpzone_" .. storage.warpzone, wx)
				if (nplanet) then game.print({ "warptorio.successful_warp" }) end
			end
		end
		if (not nplanet) then
			nplanet = remote.call("planetorio", "SimplePlanetRoll", "warpzone_" .. storage.warpzone,
				{ zone = storage.warpzone, prevplanet = vplanet })
		end -- planetorio, modifiers={{"",stuff}})
	end
...
function warptorio.Warp(cf, f) -- Find and clone entities to new surface
	...
	for k, v in pairs(etbl) do
		if (table.HasValue(warptorio.GetWarpBlacklist(), v.name)) then
			table.insert(blacktbl, v)
			etbl[k] = nil
		end
	end
	...
	if (settings.global["warptorio_robot_warping"].value == true) then
		local lgn = {}
		for k, v in pairs(etbl) do
			if (v.type == "roboport") then
				local g = v.logistic_network
				if (g and g.valid) then table.insertExclusive(lgn, g) end
			end
		end
	...
	if (remote.interfaces["aai-programmable-vehicles"]) then
		local rmt = "aai-programmable-vehicles"
		for k, v in pairs(new_ents) do
			if (isvalid(v.source) and isvalid(v.destination)) then
				local sig = remote.call(rmt, "get_unit_by_entity", v.source)
				if (sig) then remote.call(rmt, "on_entity_deployed", { entity = v.destination, signals = sig.data }) end
			end
		end
	end
	...
	for k, v in pairs(game.players) do
		if (v and v.valid) then
			local iv = v.get_main_inventory()
			if (iv) then
				for i, x in pairs(iv.get_contents()) do
					if (x.name:sub(1, 25) == "warptorio-teleporter-gate") then iv.remove { name = x.name, count = x.count } end
					if (x.name:sub(1, 20) == "warptorio-harvestpad") then if (x.count > 1) then iv.remove { name = x.name, count = (x.count - 1) } end end
				end
			end
		end
	end
...
function warptorio.SurfaceIsWarpzone(f)
	local n = f.name
