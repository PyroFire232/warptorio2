...
function warptorio.GetHarvesterLevelSize(lv)
	local z = warptorio.HarvesterSizes[lv]
	return vector(z, z)
end

function warptorio.GetHarvesterLevelSizeNum(lv)
	local z = warptorio.HarvesterSizes[lv]
	return z
end
...
local HARV = {}
HARV.__index = HARV
warptorio.HarvesterMeta = HARV
setmetatable(HARV, warptorio.TeleporterMeta)
function HARV.__init(self, tbl)
	...
	storage.Harvesters[tbl.key] = self
end
...
function HARV:GetBaseArea(z, rank)
	z = z or self:GetSize(rank) - 1
	return vector.square(self.position, vector(z, z))
end

function HARV:GetDeployArea(z, rank)
	z = z or self:GetSize(rank) - 2
	return vector.square(vector.pos(self.deploy_position), vector(z, z))
end
		...
		if (v and v.valid) then
			...
			if (iv) then
				for i, x in pairs(iv.get_contents()) do
					if (x.name:sub(1, 20) == "warptorio-harvestpad" and x.name:sub(22, 22 + self.key:len()) == self.key) then
						local lvx = tonumber(x.name:sub(-1, -1))
						if (lvx < lv) then
							iv.remove { name = x.name, count = x.count }
							iv.insert { name = cls, count = 1 }
						elseif (x > 1) then
							iv.remove { name = x.name, count = (x.count - 1) }
						end
					end
				end
			end
...
function HARV:MakePointTeleporter(tps, i, t, pos)
	...
	if (t.prototype) then
		...
		if (not isvalid(e)) then
			...
			local varea
			if (not t.gate) then
				varea = vector.square(vpos, vector(2, 2))
...
function HARV:Upgrade()
	self.ReadyUpgrade = true
	if (not self.deployed) then self:DoUpgrade() end
end

function HARV:DoUpgrade()
	if (self.ReadyUpgrade) then
		self.ReadyUpgrade = false
		self:RunUpgrade()
	end
end
...
function HARV:ConnectCombo()
	if (not self.deployed and self:ValidCombos()) then
		local red1 = self.combos[1].get_wire_connector(defines.wire_connector_id.circuit_red, true)
		local red2 = self.combos[2].get_wire_connector(defines.wire_connector_id.circuit_red, true)
		red1.connect_to(red2)
		local green1 = self.combos[1].get_wire_connector(defines.wire_connector_id.circuit_green, true)
		local green2 = self.combos[2].get_wire_connector(defines.wire_connector_id.circuit_green, true)
		green1.connect_to(green2)
		--	self.combos[1].connect_neighbour({target_entity=self.combos[2],wire=defines.wire_type.red})
		--  self.combos[1].connect_neighbour({target_entity=self.combos[2],wire=defines.wire_type.green})
	end
end

function HARV:ValidCombos() return isvalid(self.combos[1]) and isvalid(self.combos[2]) end

function HARV:CheckCombo()
	if (research.has("warptorio-alt-combinator")) then
		self:MakeComboA()
		self:MakeComboB()
		self:ConnectCombo()
	end
...
function HARV:DestroyComboA()
	if (isvalid(self.combos[1])) then entity.destroy(self.combos[1]) end
	self.combos[1] = nil
end

function HARV:DestroyComboB()
	if (isvalid(self.combos[2])) then entity.destroy(self.combos[2]) end
	self.combos[2] = nil
end

function HARV:DestroyCombos()
	self:DestroyComboA()
	self:DestroyComboB()
end

function HARV:MakeComboA()
	local vx = self.points[1].ent
	local cfg = settings.global.warptorio_combinator_offset.value
	local ofv
	if (cfg) then ofv = (self.key == "east" and 1 or 1.5) else ofv = 0 end
...
function HARV:MakeComboB()
	local vx = (isvalid(self.points[2].ent) and self.points[2].ent or self.points[1].ent)
	local cfg = settings.global.warptorio_combinator_offset.value
	local ofv
	if (cfg) then ofv = (self.key == "east" and -2 or -1) else ofv = (self.key == "east" and -1 or 1) end
	local vpos = vector.pos(vx.position) + vector(self:GetSize().x / 2 * (self.key == "east" and 1 or -1) + ofv, 0)
	vector.clean(vx.surface, vector.square(vpos, vector(0.5, 0.5)))
	local e = entity.protect(entity.create(vx.surface, "warptorio-alt-combinator", vpos), false, false)
	self.combos[2] = e
...
function HARV:MakePointLoader(tps, i, id, pos, f, belt, lddir, belty, beltsquare, vexdir)
	...
	if (not isvalid(v)) then
		...
		if v.loader_type == "output" then
			v.loader_type = "input"
		elseif v.loader_type == "input" then
			v.loader_type = "output"
		end
		self.loaders[i][id] = v
...
function HARV:CheckPointLogistics(i)
	...
	local ldodir = (i == 2 and string.compassdef[tps.logs_pattern] or string.compassdef[string.compassopp[tps.logs_pattern]])
	local lddir = (i == 1 and string.compassdef[tps.logs_pattern] or string.compassdef[string.compassopp[tps.logs_pattern]])
	...
	if (tps.dopipes and lvLogs > 0) then
		for id = 1, math.min(lvLogs, 2), 1 do
			...
			self:MakePointPipes(tps, i, id, pos, f, lddir, belty, vdir)
		end
	end
end
...
function HARV:Recall(bply) -- recall after portal is mined
	...
	local tps = self:Data()
	if (not self.deployed) then
		self:DestroyPointTeleporter(1, false)
	...
	local tbs = {}
	local tcs = {}
	for k, v in pairs(hf.find_tiles_filtered { area = harvArea }) do
	...
	local dcs = {}
	for k, v in pairs(hf.find_decoratives_filtered { area = self:GetBaseArea(nil, true) }) do
		local vpos = vector.add(vector.sub(v.position, self.position), self.deploy_position)
	...
	local ecs = {}
	for k, v in pairs(hf.find_entities_filtered { area = harvArea, type = "character", invert = true }) do
		if (v and v.valid and v ~= self.points[1].ent and v ~= self.points[2].ent and not entity.shouldClean(v) and not cache.get_entity(v) and v.type ~= "resource") then
			table.insert(ecs, v)
		end
	end
	...
	for k, v in pairs(ebs) do
		if (table.HasValue(warptorio.GetModTable("harvester_blacklist"), v.name)) then
			table.insert(blacktbl, v)
			ebs[k] = nil
		end
	end
	...
	hf.clone_entities { entities = ecs, destination_surface = f, destination_offset = { self.deploy_position.x - self.position.x, self.deploy_position.y - self.position.y }, snap_to_grid = false }
	local hfe = warptorio.Cloned_Entities
	...
	warptorio.Cloned_Entities = {}
	warptorio.IsCloning = true
	f.clone_entities { entities = ebs, destination_surface = hf, destination_offset = { self.position.x - self.deploy_position.x, self.position.y - self.deploy_position.y }, snap_to_grid = false }
	local fe = warptorio.Cloned_Entities
	warptorio.IsCloning = false
	warptorio.Cloned_Entities = nil

	local hfm = {}
	for k, v in pairs(hfe) do
		if (isvalid(v.source) and isvalid(v.destination) and table.HasValue(ecs, v.source)) then
			table.insert(hfm, v.source)
		end
	end
	local fm = {}
	for k, v in pairs(fe) do
		if (isvalid(v.source) and isvalid(v.destination) and table.HasValue(ebs, v.source)) then
			table.insert(fm, v.source)
		end
	end
	...
	if (bply) then -- players now
		local tpply = {}
		for k, v in pairs(game.players) do
			if (v.character == nil or (v.surface == f and vector.inarea(v.position, self:GetDeployArea(nil, true)))) then
				table.insert(tpply,
					{ v, vector.add(vector.add(vector.mul(self.deploy_position, -1), vector.pos(v.position)),
						self.position) })
			end
		end
		for k, v in pairs(tpply) do
			v[1].teleport(f.find_non_colliding_position("character", { v[2][1], v[2][2] }, 0, 1),
				hf)
		end
	end
	...
	self:DestroyComboB()
	self:DestroyPointTeleporter(1, false)
	self:DestroyPointTeleporter(2, false)
	self:CheckTeleporterPairs()
...
function HARV:Deploy(surf, pos) -- deploy over a harvester platform
	...
	local f = surf
	if (f ~= warptorio.GetMainSurface()) then
		game.print({ "warptorio.harvester-placement-error" })
		return false
	end
	...
	local tcs = {}
	for x = planetArea[1][1], planetArea[2][1] do
		for y = planetArea[1][2], planetArea[2][2] do
			local v = f.get_tile(x, y)
			local vpos = vector.add(vector.sub(vector(x, y), self.deploy_position), self.position)
			table.insert(tcs, { name = v.name, position = vpos })
		end
	end
	local dcs = {}
	for k, v in pairs(f.find_decoratives_filtered { area = self:GetDeployArea(self:GetSize() - 3) }) do
		local vpos = vector.add(vector.sub(v.position, self.deploy_position), self.position)
		table.insert(dcs, { name = v.decorative.name, position = vpos, amount = v.amount })
	end
	local ecs = {}
	for k, v in pairs(f.find_entities_filtered { area = planetArea, type = { "construction-robot", "logistic-robot", "character" }, invert = true }) do
		if (v.type ~= "resource" and v.name:sub(1, 9) ~= ("warptorio")) then
			table.insert(ecs, v)
		end
	end
	...
	warptorio.Cloned_Entities = {}
	warptorio.IsCloning = true
	--	f.clone_entities { entities = ecs, destination_surface = hf, destination_offset = vector.mul(vector.sub(self.position, self.deploy_position), -1), snap_to_grid = false }
	f.clone_entities { entities = ecs, destination_surface = hf, destination_offset = { self.deploy_position.x - self.position.x, self.deploy_position.y - self.position.y }, snap_to_grid = false }
	local fe = warptorio.Cloned_Entities
	warptorio.IsCloning = false
	warptorio.Cloned_Entities = nil
	...
	hf.clone_entities { entities = ebs, destination_surface = f, destination_offset = { self.deploy_position.x - self.position.x, self.deploy_position.y - self.position.y }, snap_to_grid = false }
	local hfe = warptorio.Cloned_Entities
	warptorio.IsCloning = false
	warptorio.Cloned_Entities = nil
	...
	for k, v in pairs(fm) do
		if (isvalid(v) and v ~= self.points[1].ent and v ~= self.points[2].ent) then
			entity.destroy(v)
		end
	end
	for k, v in pairs(hfm) do
		if (isvalid(v) and v ~= self.points[1].ent and v ~= self.points[2].ent) then
			entity.destroy(v)
		end
	end
